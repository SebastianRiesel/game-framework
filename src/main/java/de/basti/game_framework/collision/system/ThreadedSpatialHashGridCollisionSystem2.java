...
public class ThreadedSpatialHashGridCollisionSystem2<T extends Collider> extends CollisionSystem<T> {

	private Set<T> allColliders = new HashSet<>();
	private SpatialHashGrid<T> grid;
	private double cellWidth;

	public ThreadedSpatialHashGridCollisionSystem2() {
		this(50, 50d);

	}

	public ThreadedSpatialHashGridCollisionSystem2(int capicity) {
		this(capicity, 50d);

	}

	public ThreadedSpatialHashGridCollisionSystem2(double cellWidth) {
		this(50, cellWidth);

	}

	public ThreadedSpatialHashGridCollisionSystem2(int capacity, double cellWidth) {
			...
			for (T c1 : colliders) {
				...
				Future<Set<CollisionPair<T>>> future = exec.submit(new Callable<>() {
					...
					public Set<CollisionPair<T>> call() throws Exception {
						Set<CollisionPair<T>> collisionsWithC1 = new HashSet<CollisionPair<T>>();
						for (T c2 : colliders) {
							if (c1 == c2) {
								continue;
							}
							...
							if (c1.collidesWith(c2) || c2.collidesWith(c1)) {
								collisionsWithC1.add(new CollisionPair<T>(c1, c2) );
							}
						}
						return collisionsWithC1;
					}
